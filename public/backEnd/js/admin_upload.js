...
function placehlimage() {
    var file = document.getElementById("placehlimage_upload").files[0];
    if (file) {
        if (file.type == "image/jpeg" || file.type == "image/png" || file.type == "image/gif" || file.type == "video/x-flv" || file.type == "video/mp4" || file.type == "application/x-mpegURL" || file.type == "video/MP2T" || file.type == "video/3gpp" || file.type == "video/quicktime" || file.type == "video/x-msvideo" || file.type == "video/x-ms-wmv") {
            var img = new Image();
            ...
            img.onload = function() {
                ...
                window.URL.revokeObjectURL(img.src);
                    ...
                    document.getElementById("placehlimage_title").style.color = "green";
                    document.getElementById("placehlimage_title").innerHTML = file.name;
              
            };
        } else {
            document.getElementById("placehlimage_title").style.color = "red";
            document.getElementById("placehlimage_title").innerHTML =
                "Invalid file type";
            document.getElementById("placehlimage_upload").value = null;
        }
    }
}
