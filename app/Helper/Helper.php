...
function convert_for_paypal($amount) {
    $paypal_settings=InfixPaymentGatewaySetting::find(1)->env_terms;
  $terms= explode(',',$paypal_settings);
  $paypal= explode(':',$terms[2]);
  $paypal_currency=$paypal[1];
  // return $paypal_currency;
  $from_currency= GeneralSetting()->currency;
    if ($paypal_currency!='') {
        $data = convertCurrency($from_currency,$paypal_currency,$amount);
    } else {
        $data = convertCurrency($from_currency,'USD',$amount);
    }
    session()->put('paypal_payable_amount', number_format($data, 2, '.', ''));
    return number_format($data, 2, '.', '');
 }
...
// get GA info
if( ! function_exists('googleAnalyticsData')){
  function googleAnalyticsData(){ 
      $data = ThirdPartyApi::first();
      return $data;
  }
}
