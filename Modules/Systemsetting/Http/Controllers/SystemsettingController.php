...
class SystemsettingController extends Controller
{
    ...
    public function googleAnalytics(Request $request){
        ...
        try {
            ...
            $third_party_api=ThirdPartyApi::first();
            $access_key=env('access_key');
            return view('systemsetting::google_analytics',compact('view_id','access_key','third_party_api'));
        } catch (\Exception $e) {
            $msg=str_replace("'", " ", $e->getMessage()) ;
            Toastr::error($msg, 'Failed');
            return redirect()->back();
    ...
    public function google_analytics_setup(Request $request){
        ...
        $request->validate([
            'site_tag_id' => 'required',
            'ga_service_account' => 'required',
        ]);
        ...
        if ($request->view_id=='') {
            ...
            try {
                ...
                $third_party_api=ThirdPartyApi::first();
                $third_party_api->ga_view_id=$request->view_id;
                $third_party_api->ga_site_tag_id=$request->site_tag_id;
                

                if ($request->file('ga_service_account') != "") {
                    $file = $request->file('ga_service_account');
                    $analytics_file = "service-account-credentials." . $file->getClientOriginalExtension();
                    $file->move('app/analytics/', $analytics_file);
                    $analytics_file = 'app/analytics/' . $analytics_file;

                    $third_party_api->ga_service_account=$analytics_file;
                }
