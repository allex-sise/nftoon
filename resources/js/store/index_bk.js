...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async toonMintNftMultiple({dispatch,state}){
      console.log('DISPATCH');
      await dispatch("loadSecrets");

      //todo: salveaza in DB user + pending + luat credite
      const payload = {
        route: '',
      }
      await dispatch("storeInDb", payload);
      //mint wallet nftoon
      //todo: salveaza in DB datele NFT-ului

      try {
        const provider = new ethers.providers.Web3Provider(ethereum);
        const walletMnemonic = ethers.Wallet.fromMnemonic(state.secrets.mnemonic);
        ...
        const mintTxn = await connectedContract["mint(string)"]('QmTUX9mwjT3oHC4P3zEoHdSt1tNJrxTRib2i1nNnraxFmR');
        const payloadTxHash = {
          route: '',
          thHash: mintTxn
        }
