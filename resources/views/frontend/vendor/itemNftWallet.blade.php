 ...
 <div class="no-bottom no-top" id="content">
                        ...
                        <div class="container">
                            <div class="row">
                                ...
                                <div class="col-md-12 text-center">
									<h1>Muta in wallet-ul tau NFT-ul - {{isset($data['edit'])? $data['edit']->title:old('title')}}</h1>
                                </div>
            ...
            @if ( $data['edit']->nft_scos_wallet == 0)
                @if(Auth::user()->role_id == 1 || Auth::user()->role_id == 2 || Auth::user()->role_id == 4)
                    <form action="{{ route('author.itemNftWalletUpdate')}}" id="file_update" method="POST" enctype="multipart/form-data">
                @else
                    ...
                    @if($walletz)  
                            <form id="form-create-item" class="form-border" method="post" action="email.php">
                                <div class="field-set" style="background-size: cover;">
                                    ...
                                    @else
                                    <button  class="btn-main" id="itemSubmit" type="submit" style="width: 100%;">Muta NFT-ul in Wallet-ul tau</button>
                                    @endif
                                    <div class="spacer-single" style="background-size: cover;"></div>
                                </div>
                            </form>
                ...
                @else
                    <p> Ai deja o cerere de scoatere a nft-ului in executie</p>
                    <form action="{{ route('author.itemNftWalletReverseUpdate')}}" id="file_update" method="POST" enctype="multipart/form-data">
                    @csrf
                        <input type="text" hidden name="category_id" value="{{@$data['edit']->category_id}}">
                        <input type="text" hidden name="id" value="{{@$data['edit']->id}}">
                        <input type="text" hidden value="1" name="upload_or_link">
                        <button  class="btn-main" type="submit" style="width: 100%;">Elimina cerearea de mutare Wallet-ul tau</button>
                    </form>
                @endif
                @endif
            </section>
